...
def do_autopaginate(parser, token):
    ...
    if len(split) == 2:
        ...
        return AutoPaginateNode(
            split[1], paginate_by=split[2], context_var=context_var)
    elif len(split) == 4:
        try:
            orphans = int(split[3])
        except ValueError:
...
class AutoPaginateNode(template.Node):
    ...
    def render(self, context):
        ...
        show_line = context['request'].show_line
        if self.paginate_by is None:
            paginate_by = show_line or DEFAULT_PAGINATION
        elif isinstance(self.paginate_by, int):
            paginate_by = self.paginate_by
        else:
            paginate_by = self.paginate_by.resolve(context)

        #print 'paginate_by==', paginate_by
        ...
        context['pageObj'] = page_obj
        context['pageCounter'] = (page_obj.number - 1) * paginate_by
        return u''
...
def paginate(context, window=DEFAULT_WINDOW, hashtag=''):
    ...
    try:
        ...
        to_return = {
            ...
            'paginator': paginator,
            'hashtag': hashtag,
            'isPaginated': paginator.count > paginator.per_page,
        }
...
register.inclusion_tag(
    'pagination/pagination.html', takes_context=True
)(paginate)
